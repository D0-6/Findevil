...
async def analyst_node(state: AgentState, tools, llm_with_tools) -> AgentState:
...
async def tool_executor_node(state: AgentState, session) -> AgentState:
    ...
    for tool_call in last_msg.tool_calls:
        ...
        try:
            ...
            # Parse the content from CallToolResult
            text_content = ""
            for content in mcp_result.content:
                if content.type == "text":
                    text_content += content.text
            result = json.loads(text_content) if text_content.strip().startswith("{") or text_content.strip().startswith("[") else {"output": text_content}
            result_data = result
...
async def build_graph(session, tools):
    llm = ChatOpenAI(
        model=MODEL_NAME,
    ...
    graph.add_node("analyst", partial(analyst_node, tools=tools, llm_with_tools=llm_with_tools))
    graph.add_node("tool_executor", partial(tool_executor_node, session=session))
...
async def main():
    ...
    server_env.update({
        "SIFT_CASES_DIR": str(Path(args.image).resolve().parent),
        "SIFT_MAX_ROWS": "500",
        "MOUNT_BASE": "/tmp/sift_evidence",
        "SIFT_MOUNT_BASE": "/tmp/sift_evidence"
    })
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            graph = await build_graph(session, tools)
