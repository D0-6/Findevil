...
```
find-evil/
├── mcp_server.py          # 25-tool MCP server (final merged)
├── agent_loop.py                # LangGraph self-correcting agent
├── bench.py             # Benchmark engine
...
Run `accuracy_report.py` after `bench.py` to auto-generate the required Devpost submission accuracy report. It covers:
