"""
mcp_server.py — Find Evil Hackathon
...
TIMEOUT_SEC = int(os.environ.get("SIFT_TIMEOUT",  "1200"))
...
async def _supertimeline(args,t0):
    ...
    cmd.append(ip); _run(cmd,timeout=1200)
...
async def _yara_scan(args,t0):
    ...
    cmd=["yara"]
    if rec: cmd.append("-r")
    cmd+=[rp,tp]; out,_,rc=_run(cmd,timeout=1200)
...
async def _browser_forensics(args,t0):
    ...
    out,_,rc=_run(cmd,timeout=1200)
...
async def _file_carve(args,t0):
    ...
    out,_,rc=_run(cmd,timeout=1200)
    if rc!=0: cmd=["scalpel","-o",od,ip]; _run(cmd,timeout=1200)
    carved=[]
    try:
...
async def _sigma_matcher(args: dict, t0: float) -> dict:
    ...
    if run_sigma_matcher is None:
        return {"error": "innovations.py not found — place it in same directory as mcp_server.py"}
